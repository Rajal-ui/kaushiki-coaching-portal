  ...
  async function handleSignup(e: FormEvent) {
    ...
    try {
      const res = await fetch('/api/auth/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, phone, otp: otpCode }),
      });
          ...
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full h-12 px-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none text-base transition-colors"
                required
              />
            </div>
            <div>
