...
export function FeeSummaryCards({ enrollments }: { enrollments: Enrollment[] }) {
...
export function FeeTable({ enrollments }: { enrollments: Enrollment[] }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
        ...
        <span className="text-lg font-bold text-gray-900">
          ₹{enrollments.reduce((sum, enr) => sum + (enr.payment?.status === 'COMPLETED' ? enr.payment.amount : 0), 0).toLocaleString('en-IN')}
        </span>
      </div>
    </div>
  );
...
export function FeeSummaryCard({ enrollments }: { enrollments: Enrollment[] }) {
  if (enrollments.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <p className="text-gray-400 text-center">No enrollments found.</p>
      </div>
    );
  }

  const totalPaid = enrollments.reduce((sum, e) => sum + (e.payment?.status === 'COMPLETED' ? e.payment.amount : 0), 0);
  const pendingCount = enrollments.filter(e => e.payment?.status === 'PENDING').length;

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      <h3 className="font-semibold text-gray-900 mb-3">Fees Overview</h3>
