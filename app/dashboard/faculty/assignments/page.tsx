...
import { Loader2, Plus, FileText, Calendar, Clock, Users, BookOpen, AlertCircle, CheckCircle, ChevronRight } from 'lucide-react';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create and manage assignments for your batches</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Resources (optional)</label>
            <div className="flex gap-2 mb-2">
              <input value={newResName} onChange={e => setNewResName(e.target.value)} placeholder="File name"
                className="flex-1 h-10 px-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm" />
            ...
            {resources.map((r, i) => (
              <div key={i} className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-lg px-3 py-1.5 mb-1">
                <FileText className="w-4 h-4 text-primary" /> <span className="font-medium">{r.name}</span>
                <span className="text-xs text-gray-400 truncate flex-1">{r.url}</span>
                <button type="button" onClick={() => removeResource(i)} className="text-error hover:text-red-700 text-xs font-medium">Remove</button>
              </div>
            ))}
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-400">No assignments yet. Create your first one!</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                      ...
                      <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" /> {a.batches.map(b => b.batch.subject.name).join(', ')}</span>
                    </div>
                  </div>
