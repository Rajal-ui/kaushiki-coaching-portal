...
describe('testScorecardTemplate', () => {
  ...
  it('uses a parent-specific greeting when recipient is a parent', () => {
    const { html, text } = testScorecardTemplate({ ...base, recipientRole: 'PARENT' });
    expect(html).toContain('Your ward');
    expect(text).toContain("Your ward's scorecard");
  });
  ...
  it('formats non-integer percentages to two decimals', () => {
    const { html } = testScorecardTemplate({ ...base, score: 7, totalMarks: 30 });
